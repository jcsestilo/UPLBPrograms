...
void printGraph(GRAPH *G) {
  ...
  for (i=1; i<=G->num_vertices; i++) {
    ...
    //insert a missing loop to print neighbors of vertex i
    //see expected.txt for the expected result
    //solution is provided in the answer subfolder
    while(temp){ // while u has neighbors
      printf("%d", temp->v); // print v, or the neighbor
      temp = temp->next; // 
      if (temp) printf(", ");
      else break;
    }
