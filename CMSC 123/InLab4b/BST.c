...
void clear(BST* B){
	...
	//while(!isEmpty(B)){ delete(B,B->root->key); showTree(B); }					
...
Aside from the two cases for the child pointer that either it is NULL or it points to the only child, you also 
...
*/
	// Step 1
	BST_NODE* todel = search(B, key);
	BST_NODE* child;
	if (todel == NULL) return;

	// Step 2: Case where there are 2 children
	if(todel->left != NULL && todel->right != NULL){
		BST_NODE* descSuccessor = descendantSuccessor(todel);
		todel->key = descSuccessor->key;
		todel = descSuccessor;
	}

	// Step 3: Case when there are 1 or 0 children
	// 3.a.
	if(todel->right != NULL){
		child = todel->right;
	...
	// 3.b.
	if(B->root == todel){ // todel is also pointing to the root node
		B->root = todel->left;
	} else { // or NOT; todel is either a left or right child of its parent
		if(todel->parent->left == todel){
			// Change the left child of the parent from todel to todel-child
			todel->parent->left = child;
		} else if(todel->parent->right == todel){
			// Change the right child of the parent from todel to todel-child
...
BST_NODE* minimum(BST_NODE *node){
   ...
   //Insert your answers from the Pre-Lab Exercise
   	BST_NODE* p=node;
