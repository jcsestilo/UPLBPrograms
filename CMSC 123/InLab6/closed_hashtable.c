...
//InLab6 Exercise INSTRUCTIONS:
// h1 (Linear Probing) and h3 (Double Hashing) are already given, and you are not allowed to change them
// implement h2 - Quadratic Probing
// Re-implement all the functions affected by h2
...
//InLab6 Exercise
//You may start with the solutions to PreLab6 Exercise
	int index=0;
	if(!isFull(H)){
		switch(H->type){
			...
			case 2: // Quadratic Probing
				index=0;
				while(H->array[h2(key, H->m, index)] > 0){
					index++;
				}
				H->array[h2(key, H->m, index)] = key;
				break;
...
//You may start with the solutions to PreLab6 Exercise
	...
	if(!isEmpty(H)){
		switch(H->type){
			...
			case 2:
				i=0;
				while(H->array[h2(key, H->m,i)]>0){
					if(H->array[h2(key, H->m, i)] == key) break;
					i++;
				}
				return(H->array[h2(key, H->m, i)]==key?h2(key,H->m,i):-1);
			case 3: 
