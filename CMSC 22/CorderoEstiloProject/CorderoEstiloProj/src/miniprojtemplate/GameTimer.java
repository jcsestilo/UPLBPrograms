...
public class GameTimer extends AnimationTimer{
	...
	/* attributes for the sound clips
	 * .toExternalForm(): need kasi ni audioclip ng string form ng url, para maconvert yung url to a string, gagamitin to
	*/
	private AudioClip killedApple = new AudioClip(getClass().getResource("/audio/killedapple.mp3").toExternalForm());
	private AudioClip killedOrange = new AudioClip(getClass().getResource("/audio/killedorange.mp3").toExternalForm());
	private AudioClip getPowerUp = new AudioClip(getClass().getResource("/audio/powerup.mp3").toExternalForm());
	private AudioClip gameLose = new AudioClip(getClass().getResource("/audio/gameover.mp3").toExternalForm());
	private AudioClip gameWin = new AudioClip(getClass().getResource("/audio/youwin.mp3").toExternalForm());
	private AudioClip bgMusic = new AudioClip(getClass().getResource("/audio/backgroundmusic.mp3").toExternalForm());
	...
	public void handle(long currentNanoTime) {
		...
		double currentSec = (currentNanoTime - this.endGame) / 1000000000;
...
//		the user loses.
		if(currentSec > GameTimer.GAME_TIME || !this.myNinja.isAlive()){
			this.stop();
			bgMusic.stop(); // stop the playing background music
	...
	private void powerUpCheckCollission(){
		for(int i=0; i < this.powerups.size(); i++){
			...
			if(p.collidesWith(this.myNinja)){
				getPowerUp.play(); // play the audio for getting the power up
			...
			if(s.getVisible()){
				...
				if(s.collidesWith(orange) && orange.isVisible()){
					...
					if(orange.getHealth() <= 0){
						// play the audio for killing orange
						killedOrange.play();
				...
				for(int j = 0; j < this.apples.size(); j++){
					...
					if(s.collidesWith(apples)){ //if the sword collides with a apple
						// play the audio for killing apple
						killedApple.play();
