...
public class GameTimer extends AnimationTimer{
	...
	GameTimer(GraphicsContext gc, Scene theScene){
		...
		this.startSpawnBoss = this.startSpawnItems = this.endDestroyItems = this.startSpawnThreeApples = System.nanoTime();
		// System.out.println("INITIAL SHIP STRENGTH: " + this.myShip.getShipStrength());
	}
	...
	public void handle(long currentNanoTime) {
		...
		if(spawnBossElapsedTime > GameTimer.BOSS_SPAWN_DELAY){
			...
			// No need for resetting kasi hindi naman magsaspawn si orange ulit
			// this.startSpawnBoss = System.nanoTime();
		}
		...
		for(int i = 0; i < sList.size(); i++){
			...
			if(s.getVisible()){
				...
				if(s.collidesWith(orange) && orange.isVisible()){
					...
					//System.out.println("Orange Health: " + orange.getHealth());
				...
				for(int j = 0; j < this.apples.size(); j++){
					...
					if(s.collidesWith(apples)){ //if the sword collides with a apple
						...
						this.setScoreCounter(this.getScoreCounter() + 1); //adds 1 to the score
		...
		if(orange.getVisible()){
			...
			if(this.myNinja.collidesWith(orange)){
				...
				if(this.myNinja.isImmortal()){
					...
					// System.out.println("Ninja Strength: " + this.myNinja.getNinjaStrength());
			...
			if(a.getVisible()){
				...
				if(this.myNinja.collidesWith(a)){
					...
					if(this.myNinja.isImmortal()){ // If ninja is immortal, no reduction of health
						...
						// Print current ship strength
						// System.out.println("CURRENT SHIP STRENGTH: " + this.myShip.getShipStrength());
						...
						if(this.myNinja.getNinjaStrength() <= 0){
							...
							this.stop();
							// Should display another screen saying that the player lost
						}
