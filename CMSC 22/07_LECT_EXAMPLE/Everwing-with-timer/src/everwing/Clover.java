...
public class Clover extends Item {
	private UpgradeTimer timer;
	
	public Clover(String type){
		super(type);
	}

	void affect(Guardian g){
		g.setBulletType(Bullet.UPGRADED_BULLET);
		
		// ADDED CODE: Create and start the bulletTimer
		this.timer = new UpgradeTimer(g);
		timer.start();
