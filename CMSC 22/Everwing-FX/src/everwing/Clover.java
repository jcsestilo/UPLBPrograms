...
class Clover extends Item {
	private final static Image CLOVER_IMAGE = new Image("images/clover.png");

	Clover(double x, double y){
		super(x,y,Clover.CLOVER_IMAGE);
	}

	@Override
	void checkCollision(Guardian guardian){
		if(this.collidesWith(guardian)){
			System.out.println(guardian.getName() + " has collected a clover!");
			this.vanish();
			guardian.upgradeBullets();
		}
	}
