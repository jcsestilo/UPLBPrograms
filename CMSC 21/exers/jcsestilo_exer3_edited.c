...
int prime(int i, int tempN){ // check if prime
    if(tempN == i) return 0; 
    else if(tempN % i == 0) return 1;
    else{prime(i+1,tempN);} 
    
}
int printLargest(int primeArr[]){ // prints the largest
...
int main()
{
    ...
    else if (x==y&& prime(2,x)==0) // if x and y is equal and prime, prints x
    {
    ...
    for (x; x <= y; x++)
    {
        if (prime(2,x)==0)
        {
            primeArr[counter] = x;
            counter++;
            if(counter==3) counter=0; //if counter is 3, will reset to the first index
        }
